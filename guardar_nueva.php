...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($id_tarea)) {
        ...
        if ($completada === 'Sí') {
            ...
            if (str_contains($p_lower, 'diaria')) { $fecha_base->modify('+1 day'); }
            elseif (str_contains($p_lower, 'semanal')) { $fecha_base->modify('+7 days'); }
            elseif (str_contains($p_lower, 'mensual')) { $fecha_base->modify('+1 month'); }
            elseif (str_contains($p_lower, 'trimestral')) { $fecha_base->modify('+3 months'); }
            elseif (str_contains($p_lower, 'semestral')) { $fecha_base->modify('+6 months'); }
            elseif (str_contains($p_lower, 'anual')) { $fecha_base->modify('+1 year'); }
